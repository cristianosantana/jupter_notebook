...
def _flatten_drl_sample(sample: Any) -> list[dict[str, Any]]:
    if not isinstance(sample, dict):
        return []
    out: list[dict[str, Any]] = []
    for key in ("merged_unique_order", "top", "bottom", "outliers"):
...
def merge_tool_result_into_state(
    ...
    *,
    ...
    log_sid = state.flags.get("_orion_drl_log_session_id")
    log_sid_s = str(log_sid).strip() if log_sid is not None else ""
    if raw.get("cached"):
    ...
    if not raw.get("cached") and isinstance(raw.get("drl_summary"), dict):
        append_drl_step(
            "drl_client_merge",
            {
                "cache_key_head": cache_key[:24],
                "summary_len": len(summary),
                "row_count": raw.get("row_count"),
                "dataset_id": raw.get("dataset_id"),
            },
            session_id=log_sid_s or None,
            log_dir=cfg.llm_debug_log_dir,
        )
        ins = raw.get("drl_insights")
        if isinstance(ins, list):
            state.insights = [str(x) for x in ins if isinstance(x, str)][:12]
        did = raw.get("dataset_id")
        if isinstance(did, str) and did.strip():
            state.last_dataset_id = did.strip()
            state.last_query_signature = cache_key
            rows = raw.get("rows")
            if isinstance(rows, list) and rows:
                store: Any = rows[:500]
            else:
                store = _flatten_drl_sample(raw.get("drl_sample"))
            nd = dict(state.datasets)
            nd[state.last_dataset_id] = store
            while len(nd) > 8:
                nd.pop(next(iter(nd)))
            state.datasets = nd

    fg = dict(state.flags)
    fg.pop("_orion_drl_log_session_id", None)
    state.flags = fg
    return state
...
class ToolRegistry:
    ...
    def _domain_payload(self, state: State) -> dict[str, Any]:
        ...
        if "include_raw_rows" in extra:
            include_raw = bool(extra["include_raw_rows"])
        else:
            include_raw = self._settings.tool_domain_include_raw_rows
        ...
        sid = str(state.flags.get("_orion_drl_log_session_id") or "").strip()
        if sid:
            payload["_orion_drl_log_session_id"] = sid
    ...
    def domain_tool_cache_key(self, state: State) -> str | None:
        """Chave L1/cache idêntica à usada em `execute_default_tool` para `run_domain_query` (sem executar)."""
        domain_id = str(state.flags.get("domain_query_id") or "").strip()
        if not domain_id or self._grpc is None:
            return None
        pl = self._domain_payload(state)
        return tool_key(DOMAIN_TOOL_NAME, pl)
