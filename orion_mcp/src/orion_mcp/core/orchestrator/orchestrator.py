...
class Orchestrator:
    ...
    async def _prepare_turn(
        ...
        pkey = self._tools.pending_domain_tool_cache_key(state)
        if pkey:
            state = state.model_copy(deep=True)
            state.flags = {**state.flags, "pending_domain_tool_cache_key": pkey}
        ...
        if action == Action.CALL_TOOL:
            ...
            fg = dict(s2.flags)
            sid_log = (session_id or "").strip()
            if sid_log:
                fg["_orion_drl_log_session_id"] = sid_log
            s2.flags = fg
            state = await self._tools.run_call_tool(s2)
            budget.record_tool()
