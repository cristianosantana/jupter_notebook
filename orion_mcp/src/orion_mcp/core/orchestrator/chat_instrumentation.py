...
def snapshot_state_for_instrumentation(
    ...
    *,
    ...
    pk = str(state.flags.get("pending_domain_tool_cache_key") or "").strip()
    return {
        ...
        "pending_domain_tool_cache_key_suffix": (pk[-48:] if pk else None),
        "last_dataset_id": (state.last_dataset_id or None),
