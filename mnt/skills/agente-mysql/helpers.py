...
class ResultadoCarregamento:
    ...
    def to_agent_json(self) -> Dict[str, Any]:
        if not self.sucesso:
            return {
                "agente_id": "agente-mysql", "agente_nome": "MySQL Data Loader",
...
class MySQLAgent:
    ...
    def carregar_tabela(
        ...
        if verbose: print(f"[agente-mysql] ✅ Tabela '{tabela}' encontrada.")

        total_linhas = self._contar_linhas(tabela, filtro_where)
        resultado.total_linhas = total_linhas
        if verbose:
            print(f"[agente-mysql] Total de linhas: {total_linhas:,}")
        ...
        if verbose:
            print(f"[agente-mysql] ✅ {len(df):,} linhas × {len(df.columns)} colunas → '{resultado.variavel_notebook}'")
    ...
    def carregar_multiplas_tabelas(
        ...
        if verbose:
            print(f"[agente-mysql] ✅ {len(defs)} tabela(s) validada(s): {[d.tabela for d in defs]}")

        # Conta linhas da tabela principal
        total_linhas = self._contar_linhas(tabela_principal, "")
        resultado.total_linhas = total_linhas
        if verbose:
            print(f"[agente-mysql] Linhas na tabela principal '{tabela_principal}': {total_linhas:,}")
        ...
        if verbose:
            print(f"[agente-mysql] Query gerada:\n{query}\n")
        ...
        if verbose:
            print(f"[agente-mysql] ✅ DataFrame carregado: {len(df):,} linhas × {len(df.columns)} colunas → '{variavel_notebook}'")
            if len(df) < total_linhas:
                print(f"[agente-mysql] ⚠️  Parcial: {len(df):,} de {total_linhas:,}")
    ...
    def injetar_no_namespace(self, resultado: Dict[str, Any], namespace: Dict) -> None:
        if resultado["sucesso"] and resultado["dataframe"] is not None:
            ...
            print(f"[agente-mysql] ✅ '{var}' injetado com {len(resultado['dataframe']):,} linhas.")
        else:
